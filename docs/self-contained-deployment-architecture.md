...
xray-germany (managed Xray-core; systemd: xray-germany.service; non-root)
  ...
  └─ outbound: dokodemo-door → 127.0.0.1:9002 (opaque TCP hand-off)
...
`127.0.0.1:9002`. The canonical Xray construct for "terminate a protocol and
forward the opaque TCP" is a `dokodemo-door` **outbound** with
`address=127.0.0.1, port=9002`. The inbound is the single VLESS+Reality inbound
on 443; routing sends all its traffic to that outbound. This is the
"inbound/outbound relationship" the direction asks us to determine: a
**one-inbound → one-dokodemo-door-outbound** topology, validated by
`xray run -test` and by an L5 carrier handshake test (a real `FrameAuth` over
the Reality tunnel).
...
// /etc/split-tunnel/xray-germany.json  (0600; generated, never hand-edited)
{
  ...
  "inbounds": [{
    ...
    "streamSettings": {
      ...
      "security": "reality",
      "realitySettings": {
        "dest": "<SNI>:443",            // operator-supplied, e.g. www.microsoft.com:443
        "serverNames": ["<SNI>"],
  ...
  "outbounds": [{
    ...
    "settings": { "address": "127.0.0.1", "port": 9002 }
  }],
  "routing": { "rules": [
...
- **Q7 (how Xray connects to the splitter):** the dokodemo-door outbound IS the
  connection — Xray terminates VLESS+Reality and forwards the opaque TCP stream
  to `127.0.0.1:9002`, where `germany-splitter`'s down-carrier listener runs its
  own `FrameAuth` handshake. Two auth layers, each doing its job: Reality
  authenticates/stealths the *transport*; the tunnel secret authenticates the
  *peer* (a correct Reality client with the wrong tunnel secret is still
  rejected by the splitter — defense in depth preserved).
