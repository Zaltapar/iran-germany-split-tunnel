...
| 11 | Blackhole detection | `SPLIT_LIVENESS_ROUNDS=1` (staging value), keepalive 30 s | `iptables -I INPUT -p tcp --dport <cdn> -j DROP` on Iran one-way (packets silently lost, no RST) | within `30s × (1 round + 1 priming round) + ε` the node logs carrier loss and tears the carrier down; `carrier_loss_events` +1; no hung session. (The first tick after a carrier is established is a priming round — it sends the ping but never counts a miss, because the pong cannot have been latched yet; this is what keeps `rounds=1` from declaring every freshly established carrier dead one interval after the handshake. See `TestLivenessRoundsOneNoFalsePositive`.) | 120 s |
